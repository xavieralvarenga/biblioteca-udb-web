...
<head>
    ...
    <style>
        ...
        .btn { padding: 8px 15px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; text-decoration: none; color: white; }
...
<body>
    ...
    <h2>Ejemplares de: <span style="color:#007bff;"><c:out value="${docPadre.titulo}"/></span></h2>

    <div class="card">
        <h3>Registrar Nuevo Ejemplar Físico</h3>
        <form action="${pageContext.request.contextPath}/ejemplares" method="POST" style="display: flex; gap: 15px; align-items: flex-end;">
            ...
            <div>
                ...
                <input type="text" name="codigoDeBarras" placeholder="Ej: BARRA-00123" required style="padding:8px; border:1px solid #ccc; border-radius:4px;">
            </div>
    ...
    <table>
        ...
        <tbody>
            <c:forEach var="ej" items="${ejemplares}">
                <tr>
                    <td><c:out value="${ej.idEjemplar}"/></td>
                    <td><strong><c:out value="${ej.codigoDeBarras}"/></strong></td>
                    <td><c:out value="${ej.estado}"/></td>
                    <td>
            ...
            <c:if test="${empty ejemplares}">
                <tr>
                    <td colspan="4" style="text-align:center; color:#999;">No hay ejemplares registrados para este documento.</td>
                </tr>
            </c:if>
        </tbody>
    </table>
