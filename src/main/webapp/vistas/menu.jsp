...
<head>
    ...
    <style>
        ...
        .menu-card { background: white; border: 1px solid #dee2e6; border-radius: 8px; width: 240px; padding: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); transition: transform 0.2s; text-align: center; }
        .menu-card:hover { transform: translateY(-5px); box-shadow: 0 6px 12px rgba(0,0,0,0.1); }
        .menu-card h3 { margin-top: 0; color: #007bff; }
        .menu-card p { color: #6c757d; font-size: 14px; min-height: 40px; }
        .btn-enter { display: inline-block; background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; font-weight: bold; }
        ...
        <div class="menu-grid">
            ...
            <c:if test="${sessionScope.usuarioLogueado.nombreRol == 'Encargado (Bibliotecario)'}">
                <div class="menu-card">
                    ...
                    <p>Administración de cuentas, registros de nuevos encargados, estudiantes y profesores.</p>
                    <a href="${pageContext.request.contextPath}/usuarios" class="btn-enter">Ingresar</a>
                </div>
            </c:if>
            ...
            <div class="menu-card">
                ...
                <c:choose>
                    <c:when test="${sessionScope.usuarioLogueado.nombreRol == 'Encargado (Bibliotecario)'}">
