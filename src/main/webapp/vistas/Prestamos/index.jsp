    ...
    <%@ taglib uri="jakarta.tags.core" prefix="c" %>
        ...
        <body>
            ...
            <main id="main" class="main">
                ...
                <c:if test="${param.mensaje == 'exito' || param.mensaje == 'prestamo_ok'}">
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        <i class="bi bi-check-circle me-1"></i>
                        ¡El préstamo se ha registrado de manera exitosa!
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>

                    <script>
                        if (window.history.replaceState) {
                            window.history.replaceState(null, null, window.location.pathname + "?accion=listar");
                        }
                    </script>
                </c:if>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table datatable">
                                            ...
                                            <tbody>
                                                <c:forEach var="prestamo" items="${listaDePrestamos}">
                                                    <tr>
                                                        <th scope="row">${prestamo.idPrestamo}</th>
                                                        <td>${prestamo.carnetDocenteAlumno}</td>
                                                        <td>${prestamo.nombres}</td>
                                                        <td>${prestamo.fechaPrestamo}</td>
                                                        <td>
                                                            <c:choose>
                                                                <c:when test="${prestamo.estadoGeneral == 'ACTIVO'}">
                                                                    <span class="badge bg-success">Activo</span>
                                                                </c:when>
                                                                <c:when test="${prestamo.estadoGeneral == 'PARCIAL'}">
                                                                    <span class="badge bg-info text-dark">Parcial</span>
                                                                </c:when>
                                                                <c:when test="${prestamo.estadoGeneral == 'CONDEUDA'}">
                                                                    <span class="badge bg-warning text-dark">Con
                                                                        Deuda</span>
                                                                </c:when>
                                                                <c:when
                                                                ...
                                                                <c:otherwise>
                                                                    <span
                                                                        class="badge bg-secondary">${prestamo.estadoGeneral}</span>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </td>
                                                        <td>
